 ...
 */
public class TopRatedMovies extends ListActivity {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        MenuItem item = menu.findItem(R.id.menu_top_rated);
    ...
    private void loadSearchData(String query, SearchView search){
        ...
        Cursor cursor = db.getWordMatches(query, MoviesDataSource.allColumns, MovieDBOpenHelper.TABLE_TOP_RATED);
...
//    }
    ...
    private class MyTask extends AsyncTask<String, String, List<Movie>> {
        ...
        protected void onPostExecute(List<Movie> result) {
            ...
            if (result == null) {
                Toast.makeText(TopRatedMovies.this, "Web service not available", Toast.LENGTH_LONG).show();
                return;
            }
            movieList = result;
            MoviesDataSource dataSource = new MoviesDataSource(context);
            ...
            for(Movie movie: movieList){
                dataSource.addToTopRatedMovies(movie);
            }
            dataSource.close();
            updateDisplay();
        }
